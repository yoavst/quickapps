...
import kotlinx.android.synthetic.recorder_circle_layout.*
    ...
    fun startCounting() {
        ...
        timerTask = object : StopwatchManager.Stopwatch() {
            override fun runCode() {
                timeSinceStart += 1
                runOnUiThread(r {
                    time.setText((timeSinceStart / 60 / 60).toInt().wrap() + ":" + (timeSinceStart / 60 % 60).toInt().wrap() + ":" + (timeSinceStart % 60).toInt().wrap())
                })
            }
        }
